...
void CIrrKlangSceneNode::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options)
{
	if (!out)
		return;
	...
	if (SoundEngine && options && (options->Flags & io::EARWF_FOR_EDITOR))
	{
		...
		soundNames = new const char*[count + 2];

		for (int i = 0; i<count; ++i)
			soundNames[i] = SoundEngine->getSoundSource(i)->getName();

		soundNames[count] = "";
		soundNames[count + 1] = 0;
...
s32 CIrrKlangSceneNodeFactory::getCreatableSceneNodeTypeCount()
{
	return 1;
}
...
ESCENE_NODE_TYPE CIrrKlangSceneNodeFactory::getCreateableSceneNodeType(s32 idx)
{
	if (idx == 0)
		return (ESCENE_NODE_TYPE)IRRKLANG_SCENE_NODE_ID;
...
const c8* CIrrKlangSceneNodeFactory::getCreateableSceneNodeTypeName(s32 idx)
{
	if (idx == 0)
...
//! returns type name of a createable scene node type 
const c8* CIrrKlangSceneNodeFactory::getCreateableSceneNodeTypeName(ESCENE_NODE_TYPE type)
{
	if (type == IRRKLANG_SCENE_NODE_ID)
		return irrKlangSceneNodeTypeName;
